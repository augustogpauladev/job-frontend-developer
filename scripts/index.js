const btn = document.querySelector('.btn'); //botao que chama as funcoes
const results = document.querySelector('.results');//section onde vai aparecer os videos
const info = document.querySelector('.informations');//tag onde ira aparecer a imagem da bada e as informaçoes

 const search = (event) => {  //funçao invocadora da api do youtube

  event.preventDefault();
    const apiKey = 'AIzaSyCDx-33kv3QDIPCC--8ZnHzJgGGcYFK4oI';
    const input = document.querySelector('.search').value;//aqui pega os dados do input
     info.innerHTML = '';//limpando busca anterior se houver
     results.innerHTML = '';//limpando busca anterior se houver
    let result= [];
...
const informations = (band) => {  //funçao invocadora da api TicketMaster
 
  const article = document.createElement('article');
  article.setAttribute('class', 'information');
  let information = '';//sempre q invocada limpa a busca anterior
  info.innerHTML= '<div class=loading ></div>'

  fetch(`https://app.ticketmaster.com/discovery/v2/attractions.json?keyword=${band}&apikey=uwk3vzZlQOho8rBXJHJdT9AE8Amf767z`)//requisitando a api
  .then(response => response.json())
  .then((data) => {     
      information = data._embedded.attractions[0]//dados filtrados logo abaixo construçao do html dinamico
      article.innerHTML = ` 
                          ...
                          <div class='textInformation'>
                                ...
                                <p>
                                  ...
                                  <div class='social'>
                                      ...
                                      <ul>
                                          <li>
                                            <span style='font-weight=bold'>Facebook: </span><br>
                                            <a href='https://www.facebook.com/${(information.name).replace(" ","")}' target='blank'>https://www.facebook.com/${information.name}</a>
                                          </li>

                                          <li>
                                            <span style='font-weight=bold'>Twitter: </span><br>
                                            <a href='https://twitter.com/${(information.name).replace(" ","")}' target='blank'>https://twitter.com/${information.name}</a>
                                          </li>

                                          <li>
                                            <span style='font-weight=bold'>Instagram: </span><br>
                                            <a href='https://www.instagram.com/${(information.name).replace(" ","")}' target='blank'>https://twitter.com/${information.name}</a>
                                          </li>
                                          <li><span style='font-weight=bold'>YouTube: </span><br>
                                            <a href='https://www.youtube.com/user/${(information.name).replace(" ","")}' target='blank'>https://www.youtube.com/user/${information.name}</a>
                                          </li>
                                       
                                       </ul>
                                 </div>`;
         
      info.innerHTML='';   //retirando icone de loading ou erro se houver
      info.appendChild(article);//adicionando conteudo ao html
...
btn.addEventListener('click', search);//evento de clique do botao
